...
function selectedDriverCosts(driverName) {
  const normalized = String(driverName || "").trim().toUpperCase();
  const driver = registries.drivers.find((item) => item.name === normalized);
  return {
...
function formToRecord(existingId = "") {
  ...
  const driverCosts = selectedDriverCosts(data.get("driverName"));
...
recordsList.addEventListener("click", (event) => {
  ...
  if (!button) return;
  const draft = records.find((record) => record.id === button.dataset.draftId);
  if (!draft) return;
