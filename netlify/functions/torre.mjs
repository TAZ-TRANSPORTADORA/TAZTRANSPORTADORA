...
async function scaniaGet(company, resource, params = {}, options = {}) {
  ...
  if (result.response.status === 429) {
    const error = new Error(`Limite de requisicoes da Scania atingido em ${company.empresa}.`);
    error.code = "SCANIA_RATE_LIMIT";
    throw error;
  }
...
function isRateLimitError(error) {
  return error?.code === "SCANIA_RATE_LIMIT" || String(error?.message || "").includes("Limite de requisicoes");
}
...
async function cachedFullData(options = {}) {
  const seconds = Number(env("SCANIA_CACHE_SECONDS") || 600);
  if (!Number.isFinite(seconds) || seconds <= 0) return null;
  const maxAgeSeconds = options.allowStale ? Number(env("SCANIA_STALE_CACHE_SECONDS") || 21600) : seconds;
  try {
    ...
    if (Date.now() - new Date(cached.cachedAt).getTime() > maxAgeSeconds * 1000) return null;
    return { ...cached.data, cachedAt: cached.cachedAt, cache: true, staleCache: Boolean(options.allowStale) };
  } catch {
    return null;
  }
}
...
async function loadScaniaData() {
  ...
  const maxPages = Math.max(0, Math.min(4, Number(env("SCANIA_HISTORY_MAX_PAGES") || 0)));

  const results = [];
  for (const company of companies) {
    const vehicles = await scaniaGet(company, "vehicles");
    await sleep(300);
    const positions = await scaniaGet(company, "vehiclepositions", { latestOnly: "true" });
    await sleep(300);
    const statuses = await scaniaGet(company, "vehiclestatuses", { latestOnly: "true" });
    let dailyStatuses = [];
    let positionHistory = [];
    if (maxPages > 0) {
      try {
        await sleep(500);
        dailyStatuses = await scaniaHistory(company, "vehiclestatuses", dayStart, stop, maxPages);
        await sleep(500);
        positionHistory = await scaniaHistory(company, "vehiclepositions", last24h, stop, maxPages);
      } catch (error) {
...
export default async (request) => {
  ...
  try {
    ...
    if (isRateLimitError(error)) {
      const stale = await cachedFullData({ allowStale: true });
      if (stale) {
        const filtered = filterTorreData(stale.frota, stale.medias, access);
        return json(200, {
          ok: true,
          source: "SCANIA_RFMS",
          warning: "Scania em limite de requisicoes; exibindo ultimo cache disponivel.",
          generatedAt: new Date().toISOString(),
